<?php 
 ...
 if(is_category()) {
	$archive_title = __vce('').single_cat_title('',false);
	$description = category_description();
	if(!empty($description)){
		$archive_desc = $description;
	}
} else if(is_author()) {
	if( voice_is_co_authors_active() ) {
	...
	$archive_title = __vce('latest_posts', 'voice');
...
<?php if(!empty($archive_title)) : ?>
	<div class="main-box-head">
		<h1 class="main-box-title"><?php echo wp_kses_post( $archive_title ); ?></h1>
		<?php if(!empty($archive_desc)) : ?>
			<span class="main-box-subtitle"><?php echo wp_kses_post( $archive_desc ); ?></span>
		<?php endif; ?>
	</div>
<?php endif; ?>