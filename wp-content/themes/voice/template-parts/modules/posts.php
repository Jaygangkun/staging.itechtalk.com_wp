...
<?php $module_query = vce_get_module_query( $mod, vce_module_template_is_paged() ); ?>
<?php echo vce_open_column_wrap($mod); ?>

	<div id="main-box-<?php echo esc_attr($k+1); ?>" class="main-box vce-border-top <?php echo vce_get_column_class($mod); ?> <?php echo esc_attr(vce_get_module_css_class($mod)); ?>">
	...
	<?php if ( $module_query->have_posts() ) : ?>
		...
		<?php $i = 0; while ( $module_query->have_posts() ) : $module_query->the_post(); $i++; ?>
			...
			<?php get_template_part( 'template-parts/loops/layout-'.vce_module_layout($mod, $i) ); ?>
	...
	<?php else: ?>
		...
		<?php if ( current_user_can( 'manage_options' ) ): ?>
			<p class="no-modules-msg"><?php wp_kses_post( sprintf( __( 'No posts match your criteria. Please choose different options in <a href="%s">Module Generator</a>.', 'voice' ), admin_url( 'post.php?post='.get_the_ID().'&action=edit#vce_hp_modules' ) ) ); ?></p>
		<?php endif; ?>
