...
<?php if($instance['display_avatar']) : ?>
	<?php
	 	if($author_link && $instance['link_to_avatar']){
			...
			echo wp_kses_post( $pre_avatar. get_avatar( get_the_author_meta('ID', $user_id), $instance['avatar_size'] ) . $post_avatar );
		?>
	<?php endif; ?>
...
<?php
	if ($instance['show_social_networks']) :
		echo vce_meks_author_social_networks($user_id);
	endif;
?>
...
<?php if($author_link && $instance['link_text']) : ?>
	<div class="mks_autor_link_wrap">
		<a href="<?php echo esc_url( $author_link ); ?>" class="mks_author_link"><?php echo wp_kses_post( $instance['link_text'] ); ?></a>
	</div>
<?php endif; ?>
